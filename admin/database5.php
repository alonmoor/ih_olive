...
class MyHandler implements RequestHandler{
	...
	public function handleChildrenRequest(ChildrenRequest $req){
		...
		if(!isset($attributes['forum_decID'])){
			die("error: attribute code not given");
		}
		$parentCode = $attributes['forum_decID'];
        ...
        if (!$link) {
		 ...
		 $query = sprintf("SELECT * FROM forum_dec WHERE parentForumID='%s' ORDER BY forum_decName ",  mysqli_real_escape_string($link,$parentCode));
        ...
        while ($line = mysqli_fetch_assoc($result)) {
			$code = $line["forum_decID"];
			 
			$text ="<b> $line[forum_decName]</b>";
			
		 $node = DBTreeView::createTreeNode($text, array("forum_decID"=>$code));
				 
 
		   
	       //$node->setURL(sprintf(ROOT_WWW."/admin/find3.php?mode='forumPattern'&forum_decID=%s", $code));
	     //   $node->setURL(sprintf(ROOT_WWW."/admin/find3.php?mode='forumPattern'&forum_decID=%s", ''));
	          $node->setURL(ROOT_WWW."/admin/find3.php?mode=forumPattern&forum_decID=$code", ''  );
	 	 
		//has children
          $query2 = sprintf("SELECT * FROM forum_dec WHERE parentForumID='%s' LIMIT 1", mysqli_real_escape_string($link,$code));
...
try{
 ...
 echo "<table><tr class='menu4'><td><p><b> ",build_href2("find3.php", "","", "חזרה לטופס החיפוש","class=my_decLink_root title= 'חיפוש כללי'") . " </b></p></td>\n";	
     ...
     echo "<td><p><b> ",build_href2("../admin/manager_tree.php", "","", "עץ מנהלי הפורומים","class=my_decLink_root title='עץ מנהלי הפורומים'") . " </b></p></td></tr></table>\n";
...
$rootAttributes = array("forum_decID"=>"11");
$treeID = "treev1";
$tv = DBTreeView::createTreeView(
		$rootAttributes,
		TREEVIEW_LIB_PATH, 
		$treeID);
$str="<b>כול הפורומים</b>"	;
