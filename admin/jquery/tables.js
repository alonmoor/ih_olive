...
/**************************************************************/
        	...
        	$('#users-message').removeClass().addClass('loading').css('z-index','1').css('width','80%').html('Loading...').fadeIn().fadeOut();        	
/**************************************************************/        	
    ...
    for (var page = 0; page < numPages; page++) {
      $('<span class="page-number"></span>').text(page + 1)
        .bind('click', {newPage: page}, function(event) {
          ...
          $(this).addClass('active')
            .siblings().removeClass('active');
        }).appendTo($pager).addClass('clickable').css({'float' : 'right'});
    }
    $pager.insertBefore($table)
      .find('span.page-number:first').addClass('active');
  });
...
$(document).ready(function() {
  var $authorCells = $('table.striped td:nth-child(3)');
  var $tooltip = $('<div id="tooltip" style="display:none"></div>').appendTo('body');
