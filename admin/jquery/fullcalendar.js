...
//	dayNamesShort: ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],
	...
	monthNamesShort: ['ינואר','פבואר','מרץ','אפריל','מאי','יוני','יולי','אוגוסט','ספטמבר','אוקטובר','נובמבר','דצמבר'],
...
$.fn.fullCalendar = function(options) {
	...
	this.each(function() {
		...
		if (sections) {
			header = $("<table class='fc-header' id='myDate_table' />")
				.append($("<tr/>")
							...
							if (buttonClick) {
								...
								if (icon) {
									...
									button = $("<div class='fc-button-" + buttonName + " " + tm + "-state-default'>" +
										"<a class='my_button'><span id='mySpan_button"+j+"'>" + text + "</span></a></div>");
								}
								if (button) {
									button
										.click(function() {
			...
			"<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;left:"+left+"px'>" +
				"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
					(!event.allDay && seg.isStart ?
						...
						"</span>"
					:'') +
					"<span class='fc-event-title' id='my_event_title'>" + htmlEscape(event.title) + "</span>" +
				"</a>" +
