...
else if (!empty($_REQUEST['deleteMultiTask'])) //elseif(isset ($_GET  ['deleteMultiTask'] )	)
{
    $checbox = array();
...
function prepare_duedate($duedate, $tz = null)
{
    ...
    if ($diff < -604800 && $ad[0] == $at[0]) {
        $a['class'] = 'past';
        $a['str'] = $lang->formatMD((int)$ad[1], (int)$ad[2]);
    } elseif ($diff < -604800) {
        $a['class'] = 'past';
        $a['str'] = $lang->formatYMD((int)$ad[0], (int)$ad[1], (int)$ad[2]);
    } elseif ($diff < -86400) {
        $a['class'] = 'past';
    ...
    if (isset($config['duedateformat']) && $config['duedateformat'] == 2) $a['formatted'] = (int)$ad[1] . '/' . (int)$ad[2] . '/' . $ad[0];
    elseif (isset($config['duedateformat']) && $config['duedateformat'] == 3) $a['formatted'] = $ad[2] . '.' . $ad[1] . '.' . $ad[0];
