...
//	$sql="select forum_decName from forum_dec where forum_decID=$forum_ID   " ;
//	 
...
/********************************************************/   
 ...
 ?>  
 <meta HTTP-EQUIV="REFRESH" content="0; url=treefind_etc.php"> 
 <?  
 }
 die;   
...
/********************************************************/   
 ...
 die;
	...
	//====================================================
	echo "<h1 dir=rtl>חפש החלטות/פורומים -עץ יורד</h1>\n";
	build_form();
	 
	printf("<p><br />%s </p>\n",
	build_href("dynamic_5b.php", "", "חזרה אל רשימת החלטות"));
...
function build_form($formdata=FALSE) {
	...
	// decision form
//	form_start("findtree1.php");
?>
	
<fieldset  class="my_find_fieldset" style="background: #94C5EB url(../images/background-grad.png) repeat-x; width:90%;margin-right:60px;top:500px;">  
      
     <form action="../admin/findtree1.php" method="post"  id="my_find">      
       
       <table class="myformtable" id="my_resulttable" align="center"
        style="width:85%;margin-left:20px;margin-right:50px;border:1px;bottom:50px;"> 
...
/****************************************************************************************/
	...
	$sql = "SELECT forum_decName,forum_decID FROM forum_dec ORDER BY forum_decName";
...
/**********************************************************************************************************/
form_new_line();
form_label("  כותרת ההחלטה/ההחלטות :(אים תת סעיפים)", TRUE);	
...
function build_decision_query($pattern, $decID, $catID, $page, $size,$date,$date1) {
	...
	$sql="SELECT distinct(d.decID), d.decName,d.dec_date,d.parentDecID,r.catID
	       FROM decisions d
...
//		ORDER BY decName";
        $sql="SELECT d.decID, d.decName, d.parentDecID,d.parentDecID1, d.status,d.dec_date,r.catID
		FROM decisions d  
				...
				foreach($decisionrows as $decision) {
					...
					if(array_key_exists($decision->decID, $forums)) {
						...
						foreach($forums[$decision->decID] as $forum) {
							...
							echo td2url($forum[0], "findtree1.php?forum_decID=$forum[1]");
...
//					 }
                  if( $decision->catID) {
					...
					//==========================================================================================
 					...
 					td2url($row[0]->decName, "findtree1.php?decID=$decision->parentDecID"); 
					...
					td2url($row[0]->decName, "findtree1.php?decID=$decision->parentDecID1"); 
					 //td2($decision->parentDecID1);
					} 
					if($decision->status)
					echo td1("סטטוס:"), td2($decision->status);
					if($decision->dec_date)
					echo td1("תאריך:"), td2($decision->dec_date);
		 ...
		 for($i=0; $i<$items; $i++) {
			...
			foreach($rows as $row) 
			 if($forums[$i]->forum_decID == $row->forum_decID)  {
				...
				echo td1("");
				echo td2url($row->decName, "findtree1.php?decID=$row->decID");
...
//*********************************************************************************************************/
               ...
               foreach($catName   as $catname ){ 
 	                    ...
 	                    $cID=$catname[0][1];
 	                     echo td1(""),
                         td2url($catname [0][0]  , "findtree1.php?catID=$cID"); 
                      // echo td1("", "tdinvisible"), td2("", "tdinvisible"); 
					 }
 				...
 				echo '<table class="resulttable">', "\n";
 				    if($decision->parentDecID)
 					echo td1("מקושרת להחלטה:"),
                    td2url($decision->decName, "findtree1.php?decID=$decision->parentDecID"); 
 				    td2($decision->parentDecID);
					if($decision->parentDecID1)
...
function build_cat_string($catNames, $catParents, $catID) {
	$tmp = build_href("findtree1.php", "catID=$catID", $catNames[$catID]);
...
function build_forum_string( $forum,$forum_decID) {
	...
	$tmp = build_href("findtree1.php", "forum_decID=$forum_decID", $forum[$forum_decID]);
...
function build_dec_string($decNames, $decParents, $decID) {
	$tmp = build_href("findtree1.php", "decID=$decID", $decNames[$decID]);
...
/*******************************************************************************/										
						...
						function show_page_links($page, $pagesize, $results, $query) {
							...
							if($page>1) {
								for($i=1; $i<$page; $i++)
								echo build_href("findtree1.php", $query . "&page=$i", $i), " ";
								echo "$page "; }
								if($results>$pagesize) {
									$nextpage = $page + 1;
									echo build_href("findtree1.php", $query . "&page=$nextpage", $nextpage);
								}
								echo "</p>\n";
						}
                         html_footer();
