...
class MyHandler implements RequestHandler{
	...
	public function handleChildrenRequest(ChildrenRequest $req){
		...
		if(!isset($attributes['managerTypeID'])){
			die("error: attribute code not given");
		}
		$parentCode = $attributes['managerTypeID'];
        ...
        if (!$link) {
		 ...
		 $query = sprintf("SELECT * FROM manager_type WHERE  parentManagerTypeID='%s' order by managerTypeName",
		...
		while ($line = mysqli_fetch_assoc($result)) {
			$code = $line["managerTypeID"];
			 
			$text ="<b> $line[managerTypeName]</b>" ;
			 
			
		 $node = DBTreeView::createTreeNode($text, array("managerTypeID"=>$code));
		 
	      $node->setURL(sprintf(ROOT_WWW."/admin/find3.php?mode=search_forum&managerTypeID=%s", $code));
		
		//has children
          $query2 = sprintf("SELECT * FROM manager_type WHERE parentManagerTypeID='%s'order by managerTypeName LIMIT 1",  mysqli_real_escape_string($link,$code));
...
$rootAttributes = array("managerTypeID"=>"11");
$treeID = "treev1";
$tv = DBTreeView::createTreeView(
		$rootAttributes,
		TREEVIEW_LIB_PATH, 
		$treeID);
$str=" סוגי מנהלים- עין השופט"	;
