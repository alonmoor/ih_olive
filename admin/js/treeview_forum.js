...
/*****************************************************************************************************/	    
		...
		textCellElem.id =this.elem.id+j+ "nodeText";
		if(node.URL !=null){
		  ...
		  //   aElem.id=this.treeView.id+j;
		
				if ( ((this.elem.id).substring(0 ,6 )== 'treev1') ){ 
					
 					 aElem.setAttribute('class', 'href_modal_tree');
					// treeView.root.text='ערוך את ההחלטות';	
					treeView.root.text='צפה בהחלטות';	
					 aElem.className = "href_modal_tree";
		...
		//for jquery-1.4.2		
		    	...
		    	// aElem.setAttribute('class', 'href_modal_tree2');
				// aElem.setAttribute('href', '#'); 
			   // aElem.setAttribute('onClick', 'openmypage2( \''+node.URL+'\' )');
		    	if (document.implementation && document.implementation.createDocument)
			 	{
		    		  
		    		 treeView.root.text='ערוך2 את ההחלטות';	
			    	 aElem.setAttribute('onClick', 'openmypage2("' + node.URL+ '");return false;');
			    //	 aElem.setAttribute('onClick', function() { 'javascript:openmypage2("' +  aElem.href + '" );return false;' });//new  window
			    	 aElem.className = "href_modal_tree2";
			 	}
...
DBTreeView.TreeView = function(params){
	//document.getElementById(document.forms[0].id).submit();
...
//	if(this.id==='treev1' && (document.getElementsByTagName('table')[7])){
// 		 document.getElementsByTagName('table')[7].innerHTML  ="<div class=\"treeView\" id="+treeView.id+">"+treeView.loadingMessage+"</div>";
// 		 
// 	 	document.getElementsByTagName('table')[7].style.color="yellow";
// 	 
// 		document.getElementsByTagName('table')[7].text=DBTreeView_arr[0];
// 	 }
//
//	 if(this.id==='treev2' && (document.getElementsByTagName('table')[8])){ 
//		  
//  		 document.getElementsByTagName('table')[8].innerHTML ="<div class=\"treeView\" id=\""+treeView.id+"\">"+treeView.loadingMessage+"</div>";
// 		
// 		  document.getElementsByTagName('table')[8].style.color="white";
//		 
//	 }
	  
	//   if(!( document.getElementsByTagName('table')[12].innerHTML==undefined) && !( document.getElementsByTagName('table')[13].innerHTML==undefined) ){	  
	  if(this.id==='treev1' ){
		
	 		 document.getElementsByTagName('table')[14].innerHTML  ="<div class=\"treeView\" id="+treeView.id+">"+treeView.loadingMessage+"</div>";
	 		
	 	 }

	  else if(this.id==='treev2' ){ 
			 
	  		 document.getElementsByTagName('table')[15].innerHTML ="<div class=\"treeView\" id=\""+treeView.id+"\">"+treeView.loadingMessage+"</div>";
    ...
    var tvElem = document.getElementById(treeView.id)?document.getElementById(treeView.id):treeView.id;
	...
	var callback = function(response){
		...
		if(treeView.root !=null && treeView.root.text !=null){
			...
			if(treeView.root.URL !=null){
				...
				textCellElem.style.cssText = 'font-size:20px; color:purple;';//css colortext
			}
			rowElem.appendChild(textCellElem);
...
function openmypage_find(link){  
	 
	 ajaxwin=dhtmlwindow.open("ajaxbox", "ajax", link, "× ×ª×× ×× ××××¢×¨××ª", "width=1000px,height=600px,left=100px,top=100px,resize=1,scrolling=1");
