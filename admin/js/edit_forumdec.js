...
/*******************************************NEW_MGR_TYPE*********************************************************/
   ...
   if($('#textarea_mgrType_'+forum_decID).val()  && !($('#textarea_mgrType_'+forum_decID).val() =='none')  ){
...
$('form#forum fieldset').append('<div id="my_forum_entry_b"></div>').find('select#insert_forum').change(function(){
	 var flag_level=$('#flag_level').val();
	 if(flag_level==1){  
	 if($('#forum_decision_link').val()  && $('#newforum').val() ){return;}
	 var frmID=( $('#forum_decision_link').val());  
data_a=new Array();
data_a[0]=this.value;
data_a[1]=frmID;
...
$("#edittags1").autocomplete('/alon-web/olive_prj/admin/ajax2.php?suggestuserTags', {scroll: false, multiple: true, selectFirst:false, max:8});
/*****************************************************************************************************/
...
/************************************************************************************************/	 
	 ...
	 $(function() {
	        ...
	        $("#forum_decision").bind("mousedown", function() {return false;});
	    });
	 }else{
		   $("#menu_items_forum_8").sortable(); 
