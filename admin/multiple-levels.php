...
class myHandler implements RequestHandler{
	...
	private function getChildren($parentCode){
		...
		$query = sprintf("SELECT * FROM categories_subject WHERE parentCatID='%s' AND status='1' ", 
...
//			$node->setURL(sprintf(ROOT_WWW."/admin/subject.php?type=$code&catID=%s", $code));
			...
			//has children
			$query2 = sprintf("SELECT * FROM categories_subject WHERE parentCatID='%s' AND status='1' LIMIT 1", 
