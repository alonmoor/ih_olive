...
?>
<input type="hidden" name="hidden_entry<?php echo $updateID;?>" id="hidden_entry<?php echo $updateID;?>" value="<?php echo $updateID;?>"/>
...
/*******************************************************************************************/
	...
	function read_decision_data($decID) {
		...
		if($rows[0]->dec_allowed=='public')
		$rows[0]->dec_allowed= '1';
		elseif($rows[0]->dec_allowed=='private' )
		 $rows[0]->dec_allowed= '2';
		elseif($rows[0]->dec_allowed=='top_secret' )
...
/***************************************************************************************/			
			...
			$i=0;	
			foreach($rows as $row){
				if(!$result["dest_decisionsType"][$i]){
...
if(!is_array($formdata['dest_forums']) && trim($formdata["new_forum"])==""  
	&&  !$formdata['dest_forums']  )  {
	$formdata['dest_forums']="";	
       
    }elseif(! is_array($formdata['dest_forums']) 
         && ($formdata['dest_forums']==null) || !$formdata["dest_forums"] || !array_item($formdata,'dest_forums')   
   ...
   $new_forum=explode(';',$formdata["new_forum"]);  
	...
	}elseif( $formdata["dest_forums"] && is_array($formdata["dest_forums"]) 
...
/************************************************************************************************************/	     	
  }elseif(  ($formdata["dest_forums"] && is_array($formdata["dest_forums"])  && count($formdata["dest_forums"])>0 )  
...
/************************************************************************************************************/ 
	...
	if(  (trim($formdata['dest_forums'])=="" || trim($formdata['dest_forums']=='none')) 
	   && trim($formdata["new_forum"])=="") {
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if(!(is_array($formdata['vote_level']))){
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
if(!(is_array($formdata['dec_level']))){    
...
if( (!array_item($formdata,'dec_date') && $formdata['dec_date']==null) && !is_array($dateIDs) ){
...
if(!is_array($formdata["dest_decisionsType$decID"]) && trim($formdata["new_decisionType"])==""  
	&&  !$formdata["dest_decisionsType$decID"]  )  {
	$formdata["dest_decisionsType$decID"]="";	
       
    }elseif(! is_array($formdata["dest_decisionsType$decID"]) 
         && ($formdata["dest_decisionsType$decID"]==null) || !$formdata["dest_decisionsType$decID"] || !array_item($formdata,"dest_decisionsType$decID")   
	      && !trim($formdata["new_decisionType"])=="" ){
...
/***************************************************************************************************/	
	...
	}elseif( $formdata["dest_decisionsType$decID"] && is_array($formdata["dest_decisionsType$decID"]) 
...
/************************************************************************************************************/	     	
  }elseif(  ($formdata["dest_decisionsType$decID"] && is_array($formdata["dest_decisionsType$decID"])  && count($formdata["dest_decisionsType$decID"])>0 )  
    && trim($formdata["new_decisionType"]==null || trim($formdata["new_decisionType"]=="")      ) ){	
...
/**********************************************************************/
	...
	function add_decision(&$formdata="",&$forumsIDs="",&$catsIDs="",&$dateIDs=""){
		...
		if ($formdata['insertID'])
		$insertID=$formdata['insertID'];
		elseif($_GET['insertID'])
...
/********************************************************************************************/
				...
				$form['vote_level']=$vote_level1; $x_vote++;
				
/********************************************************************************************/
				
				$form['dec_level']=$dec_level1; $y_level++;
...
/********************************************************************************************/                
			...
			if($formdata['dynamic_5b'] && (count($subcatarray)<2)  ){ 
...
/***********************************************************************************/
			...
			}elseif($formdata['dynamic_5b'] && (count($subcatarray)>=2)  ){//multiple decisions		
...
/*******************************************************************************************************/
	...
	function save_forum_ajx(&$formdata) {
	    ...
	    $decID=$formdata['decID'];	
		if ($formdata["dest_forums$decID"]=='none' && !array_item ($formdata,"dest_forums$decID") ){   
		 unset ($formdata["dest_forums$decID"]);
		$tmp= "";
	
/******************************************************************************************************/	
 
}elseif ( ($formdata["dest_forums$decID"]!='none' &&  $formdata["dest_forums$decID"]!=null
   && ( array_item ($formdata,"dest_forums$decID") &&  is_array(array_item ($formdata,"dest_forums$decID"))   ) )
   && (!array_item ($formdata,"new_forum$decID")  
			   ||  ($formdata["new_forum$decID"]=='none' )
			   ||  $formdata["new_forum$decID"]==null )  )   { 
   
   	
 $tmp= $formdata["dest_forums$decID"]  ? $formdata["dest_forums$decID"] :  $formdata["new_forum$decID"]  ; 
...
/*************************************************************************************************************/	 
}elseif ($formdata["dest_forums$decID"]!='none'  &&  $formdata["dest_forums$decID"]!=null && trim($formdata["new_forum$decID"]!='none') 
   && array_item ($formdata,"new_forum$decID")
...
function save_category_ajx(&$formdata) {
	...
	$decID=$formdata['decID'];	
		if ($formdata["dest_decisionsType$decID"]=='none' && !array_item ($formdata,"dest_decisionsType$decID") ){   
		 unset ($formdata["dest_decisionsType$decID"]);
		$tmp= "";
	
/******************************************************************************************************/	
 
}elseif ( ($formdata["dest_decisionsType$decID"]!='none' &&  $formdata["dest_decisionsType$decID"]!=null
...
/*************************************************************************************************************/	 
}elseif ($formdata["dest_decisionsType$decID"]!='none'  &&  $formdata["dest_decisionsType$decID"]!=null && trim($formdata["new_decisionType$decID"]!='none') 
   && array_item ($formdata,"new_decisionType$decID")
