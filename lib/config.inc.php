...
$contact_email = 'alonmor2@gmail.com';
...
define ('BASE_URI',ROOT_WWW. '/get_in_out2/');//('BASE_URI', '‪/alon-web/olive/admin/a_freind/ex1/');
define ('BASE_URL', '192.168.0,204');
...
function my_error_handler ($e_number, $e_message, $e_file, $e_line, $e_vars) {
	...
	if (!$live) { // Show the error in the browser.
		...
		error_log ($message, 1, $contact_email, 'From:alonmor2@gmail.com');
		
		// Only print an error message in the browser, if the error isn't a notice:
		if ($e_number != E_NOTICE) {
			echo '<div class="error">בעייה במערכת מתנצלים.</div>';
		}
...
function redirect_invalid_user($check = 'userID', $destination = 'index.php', $protocol = 'http://') {
	...
	if (!isset($_SESSION[$check])) {
		...
		$url =  ROOT_WWW."/get_in_out2/html/$destination/"; // Define the URL.
		header("Location: $url");
