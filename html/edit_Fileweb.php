...
$sql="SELECT * FROM categories_file WHERE catID=$catID ";
if($rows=$db->queryObjectArray($sql)){
$row=$rows[0];

if (!($_SERVER['REQUEST_METHOD'] == 'POST') && !(array_item($_GET, 'mode')=='del_file')  ){


 if ($catID   && !(array_item($_GET, 'mode')=='del_file') ){
...
?>
<form action="<?php print SELF ?>?mode=edit_web&catID=<?php print $catID ?>" method="post"    dir="rtl">
