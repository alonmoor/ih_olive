...
/********************************************************************************/
  $sql="SELECT * FROM categories_subject  ORDER BY catID ASC, catName ASC ";	
...
<table >
         ...
         <span >
          <a href="<?php echo $_SERVER['SCRIPT_NAME']?>?mode=add_file_item_win" style="float:right;" class="my_href_li">
...
<?php    
 if ($result) { ?>
    ...
    while ($i < 5 && $aData= mysqli_fetch_assoc($result)) {
       ...
       //$bg = "yellow";
        $bg = "#F6F6F6";
        $aState = array("act_file_win", "deact_file_win");
...
<?php     
//to try->form_list_b 
   form_list_b("fileType" , $rows2, $parentCatID,"id=fileType  style='width:130px;' ");		      
?>
    </td> 
 ...
 <td width="90" bgcolor="<?php print $bg ?>"> 
    <a class="my_href_li1"  href="<?php echo $_SERVER['SCRIPT_NAME']?>?mode=edit_file_win&catID=<?php print $aData ["catID"] ?>&parentCatID=<?php print $aData ["parentCatID"] ?>"><b>ערוך</b></a>&nbsp;|&nbsp;
    <a class="my_href_li1" href="<?php echo $_SERVER['SCRIPT_NAME']?>?mode=del_file_win&catID=<?php print $aData ["catID"] ?>" onclick="return verify();"><b>מחק</b> </a> 
 </td>
  ...
  renderPaging_files_win($start, $total); 
  ?>
